...
import UploadFilesModal, { socketContext } from "./upload";
import ky from "ky";
...
export function Terminal({ accessKey }: { accessKey: string }) {
  ...
  useEffect(() => {
    socketContext.socket = new SimpleWebSocket(
    ...
    socketContext.socket.on("error", (err) => {
      console.log(err);
    });
    ...
    socketContext.socket.on("connection", () => {
      ...
      setIsConnected(true);
      socketContext.socket?.send("authorization", accessKey);
    });
    socketContext.socket.on("disconnect", () => {
      setIsConnected(false);
    });
    socketContext.socket.on("commandline", (data, type) => {
      if (typeof data === "string") addOutputs([data], type);
      else addOutputs(data, type);
    });

    socketContext.socket.on("resources", (data) => {
      setUsage(data);
    });

    setIsConnected(socketContext.socket._socket.readyState === 1);

    return () => {
      socketContext.socket?.removeAllListeners("commandline");
      socketContext.socket?.removeAllListeners("connection");
      socketContext.socket?.removeAllListeners("disconnect");
      socketContext.socket?.removeAllListeners("resources");
      socketContext.socket?._socket.close();
    };
  }, []);
